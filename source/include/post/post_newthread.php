...
if(empty($_G['forum']['fid']) || $_G['forum']['type'] == 'group') {
	showmessage('forum_nonexistence');//判断论坛id是否存在，以及类型是否为group;
}
...
if(($special == 1 && !$_G['group']['allowpostpoll']) || ($special == 2 && !$_G['group']['allowposttrade']) || ($special == 3 && !$_G['group']['allowpostreward']) || ($special == 4 && !$_G['group']['allowpostactivity']) || ($special == 5 && !$_G['group']['allowpostdebate'])) {
	showmessage('group_nopermission', NULL, array('grouptitle' => $_G['group']['grouptitle']), array('login' => 1));//判断用户组的权限
}

if($_G['setting']['connect']['allow'] && $_G['setting']['accountguard']['postqqonly'] && !$_G['member']['conisbind']) {
	showmessage('postperm_qqonly_nopermission');//提示先绑定QQ再发帖
}


if(!$_G['uid'] && !((!$_G['forum']['postperm'] && $_G['group']['allowpost']) || ($_G['forum']['postperm'] && forumperm($_G['forum']['postperm'])))) {
	if(!defined('IN_MOBILE')) {
		showmessage('postperm_login_nopermission', NULL, array(), array('login' => 1));//判断是否登陆
	} else {
		showmessage('postperm_login_nopermission_mobile', NULL, array('referer' => rawurlencode(dreferer())), array('login' => 1));//判断是否登陆
	}
} elseif(empty($_G['forum']['allowpost'])) {
	if(!$_G['forum']['postperm'] && !$_G['group']['allowpost']) {
		showmessage('postperm_none_nopermission', NULL, array(), array('login' => 1));//判断登陆后的权限
	} elseif($_G['forum']['postperm'] && !forumperm($_G['forum']['postperm'])) {
		showmessagenoperm('postperm', $_G['fid'], $_G['forum']['formulaperm']); //判断登陆后的用户组的权限
	}
} elseif($_G['forum']['allowpost'] == -1) {
	showmessage('post_forum_newthread_nopermission', NULL); //特定用户组才能在这儿发帖
}
...
checklowerlimit('post', 0, 1, $_G['forum']['fid']); //与积分相关
...
if(!submitcheck('topicsubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	//人才供求版块二次开发代码开始
	if(!empty($_POST['infos']) && !empty($_POST['describe']) && !empty($_POST['compplace']) && !empty($_POST['contact'])  && !empty($_POST['compphone']) && !empty($_POST['mypic'])){
		$stat = array();
		$arr = array('infos','describe','compname', 'compplace', 'contact','compphone','mypic');
		$stat = getStat($arr, $_POST);
		$stat['uid'] = $_G['uid'];
		$stat = dhtmlspecialchars($stat);
		$timestamp = TIMESTAMP;

		$result = C::t('forum_koueitalent_recruit')->insert(array('uid' => "$stat[uid]", 'tid' => "$tid",
			'rec_infos' => "$stat[infos]",'rec_describe' => "$stat[describe]",
			'rec_compname' => "$stat[compname]", 'rec_compplace' => "$stat[compplace]", 'rec_contact' => "$stat[contact]",
			'rec_compphone' => "$stat[compphone]", 'rec_mypic' => "$stat[mypic]",
			'dateline' => "$timestamp"));
	}

	if(!empty($_POST['username']) && !empty($_POST['barthdate']) && !empty($_POST['gender']) && !empty($_POST['wedding']) && !empty($_POST['school_info']) && !empty($_POST['endschool']) && !empty($_POST['qqnum']) && !empty($_POST['phonenum']) && !empty($_POST['emails']) && !empty($_POST['workexperience']) && !empty($_POST['workxz']) && !empty($_POST['qwposition']) && !empty($_POST['daiyu']) && !empty($_POST['workplace']) && !empty($_POST['dgsj']) && !empty($_POST['selfeval'])){
		$arr = array('username','barthdate','gender','wedding','school_info','endschool','qqnum','phonenum','emails','workexperience','workxz','qwposition','daiyu','workplace','dgsj','selfeval');
		$stat = getStat($arr, $_POST);
		$stat['uid'] = $_G['uid'];
		$stat = dhtmlspecialchars($stat);
		$timestamp = TIMESTAMP;
		$result = C::t('forum_koueitalent_jobhunt')->insert(array('uid' => "$stat[uid]", 'tid' => "$tid",
			'job_realname' => "$stat[username]", 'job_DOB' => "$stat[barthdate]",
			'job_sex' => "$stat[gender]", 'job_mar' => "$stat[wedding]",
			'job_edu' => "$stat[school_info]", 'job_sch' => "$stat[endschool]",
			'job_myqq' => "$stat[qqnum]", 'job_phonenum' => "$stat[phonenum]",
			'job_email' => "$stat[emails]", 'job_expyear' => "$stat[workexperience]",
			'job_nature' => "$stat[workxz]", 'job_pos' => "$stat[qwposition]",
			'job_treatment' => "$stat[daiyu]", 'job_workpos' => "$stat[workplace]",
			'job_days' => "$stat[dgsj]", 'job_self' => "$stat[selfeval]",
			'dateline' => "$timestamp"
		));
		if(!empty($_POST['infos'])){
			$stat['infos'] = dhtmlspecialchars($_POST['infos']);
			$infos = C::t('forum_koueitalent_jobhunt_explog')->insert(array('tid' => "$tid", 'job_exp' => "$stat[infos]"));
		}
	}
	//人才供求版块二次开发代码结尾
...
function getStat($arr, $post){
	$stat = array();
	foreach ($arr as $key => $value) {
		$stat[$value] = $post[$value];
	}
	return $stat;
}
?>