...
if(!$operation) {
	...
	if(!submitcheck('delsubmit') && !submitcheck('undelsubmit')) {
		...
		if(submitcheck('undelsubmit')) {
			...
			//二次开发
			deletekoueitalent($threadlist, 'forum_koueitalent_recruit');
			deletekoueitalent($threadlist, 'forum_koueitalent_jobhunt');
...
<?php
	...
	if(!submitcheck('rbsubmit', 1)) {
		...
		if($deletetids) {
			require_once libfile('function/delete');
			//二次开发
			deletekoueitalent($deletetids, 'forum_koueitalent_recruit');
			deletekoueitalent($deletetids, 'forum_koueitalent_jobhunt');
