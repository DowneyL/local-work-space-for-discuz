...
class table_forum_thread extends discuz_table
{
	...
	/* 二次开发新增 */
	public function fetch_by_kouei_fid($fids, $order = null, $sort = 'DESC', $start = 0, $limit = 0, $dateline = null, $tableid = 0) {
		$fids = dintval($fids, true);

		if($fids) {
			if ($order) {
				$ordersql = ' ORDER BY '.DB::order($order, $sort);
			} else {
				$ordersql = '';
			}

			if ($dateline) {
				$wheresql = ' AND dateline > '.dintval($dateline).' AND dateline < '.getglobal('timestamp');
			} else {
				$wheresql = '';
			}
	...
	public function count_by_kouei_fids($fids, $dateline = null, $tableid = 0) {
		if ($dateline) {
			$wheresql = ' AND dateline > '.dintval($dateline).' AND dateline < '.getglobal('timestamp');
		} else {
			$wheresql = '';
		...
		if (is_array($fids)) {
			return DB::result_first("SELECT COUNT(*) FROM %t WHERE fid IN (%n) $wheresql", array($this->get_table_name($tableid), (array)$fids));
		} else {
			return DB::result_first("SELECT COUNT(*) FROM %t WHERE fid=%d $wheresql", array($this->get_table_name($tableid), $fids));
		} 
	}
