...
class table_forum_koueitalent_recruit extends discuz_table
{
	public function __construct() {
		$this->_table = 'forum_koueitalent_recruit';
		$this->_pk = 'rid';
	...
	public function delete_by_tid($tids, $unbuffered = false, $tableid = 0, $limit = 0) {
		...
		if($tids) {
			...
			C::t('forum_koueitalent_recruit')->delete_by_tids($tids);
	...
	public function update_displayorder_by_recids($recid) {
		return DB::query("UPDATE %t SET displayorder = 0 WHERE rec_id = %d", array($this->_table, $recid), $this->_pk);
	}
