...
//二次开发
function deletekoueitalent ($recyclebintids,$tablename) {
	global $_G;
	if(is_array($recyclebintids) && $recyclebintids){
		foreach ($recyclebintids as $key => $recyclebintid) {
			$kouei_result = C::t("$tablename")->fetch($recyclebintid);
			if(!empty($kouei_result)){
				if($tablename == 'forum_koueitalent_recruit'){
					if($kouei_result['rec_mypic']){
						@unlink($_G['setting']['attachdir'].'/album/'.$kouei_result['rec_mypic']);
					}
				}
				$kouei_delete = C::t("$tablename")->delete_by_tid($recyclebintid);
				if($tablename == 'forum_koueitalent_jobhunt'){
				$kouei_result_log = C::t('forum_koueitalent_jobhunt_explog')->fetch($recyclebintid);
					if(!empty($kouei_result_log)){
						$kouei_delete_log = C::t('forum_koueitalent_jobhunt_explog')->delete_by_tid($recyclebintid);
					}
				}
			}
		}
