...
if($_GET['action'] == 'checkpatch') {
	...
	//二次开发、设置应用更新提醒，只有admin才能接受到，其他管理员接受不到！
	if($_G['uid'] == 1 && $_G['member']['allowadmincp'] == 1) {
	...
	include template('common/footer_ajax');
	exit;
} elseif($_GET['action'] == 'updatenotice'){
	include template('common/header_ajax');
	loadcache('plugin');
	$vars = $_G['cache']['plugin']['kouei_notice'];
	if($vars['flag'] == 1){
		$notice_title = dhtmlspecialchars($vars['title']);
		$notice_content = dhtmlspecialchars($vars['content']);
		$notice_link = dhtmlspecialchars($vars['link']);
		$notice_point = dhtmlspecialchars($vars['point']);
		$lang = lang('forum/misc');
		echo '<div class="bm"><div class="bm_h cl"><a href="javascript:;" onclick="$(\'update_notice\').style.display=\'none\';setcookie(\'updatenotice\', 1, 86400)" class="y" title="'.$lang['patch_close'].'">'.$lang['patch_close'].'</a>';
		echo '<h2 class="i">'.$notice_title.'</h2></div><div class="bm_c">';
		echo '<div class="cl bbda pbm">'.$notice_content.'</div>';
		echo '<div class="ptn cl"><a href="'.$notice_link.'" class="xi2 y">'.$notice_point.' &raquo;</a></div>';
		echo '</div></div>';
	}
	include template('common/footer_ajax');
	exit;
