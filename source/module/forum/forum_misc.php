...
if($_GET['action'] == 'paysucceed') {
	...
	} elseif(!isset($_G['setting']['extcredits'][$_G['setting']['creditstransextra'][13]])) {
	...
	$balance = getuserprofile('extcredits'.$_G['setting']['creditstransextra'][13]);
	...
	if(!submitcheck('paysubmit')) {
		...
		if(!empty($_GET['buyall'])) {
			...
			$status = getuserprofile('extcredits'.$_G['setting']['creditstransextra'][13]) < $tprice ? 1 : 0;
		} else {
			$aids = array($aid);
		...
		if($status == 1) {
			showmessage('credits_balance_insufficient', '', array('title' => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][13]]['title'], 'minbalance' => $attach['price']));
		}
		foreach($aids as $aid) {
			$updateauthor = 1;
			$authorEarn = $prices[$aid][1];
			if($_G['setting']['maxincperthread'] > 0) {
				$extcredit = 'extcredits'.$_G['setting']['creditstransextra'][13];
				$alog = C::t('common_credit_log')->count_credit_by_uid_operation_relatedid($attach['uid'], 'SAC', $aid, $_G['setting']['creditstransextra'][13]);
			...
			if($updateauthor) {
				updatemembercount($attach['uid'], array($_G['setting']['creditstransextra'][13] => $authorEarn), 1, 'SAC', $aid);
			}
			updatemembercount($_G['uid'], array($_G['setting']['creditstransextra'][13] => -$prices[$aid][0]), 1, 'BAC', $aid);
	...
	$aid = intval($_GET['aid']);
	$extcreditname = 'extcredits'.$_G['setting']['creditstransextra'][13];
