...
if(!empty($threadids)) {
	...
	/*
	 * 二次开发新增
	 * 描述：主要用于手机版帖子表格内显示帖子的内容
	 * 时间：2017.9.4
	 */
	if( defined('IN_MOBILE') && IN_MOBILE == 2 ){

		include_once libfile('function/post');
		function kouei_urlencode($thread, $tableid, $arr, $json) {
			foreach($arr as $params){
				if($params == 'message'){
					$json["$tableid"]["$params"] = $thread["$tableid"]["$params"] = urlencode(preg_replace('/\s+/','', $thread["$tableid"]["$params"]));
				}else{
					$json["$tableid"]["$params"] = $thread["$tableid"]["$params"] = urlencode($thread["$tableid"]["$params"]);
				}
			}
			return $json;
		}
		
		$encodearr = array('tid', 'subject', 'author', 'dateline', 'message','displayorder','digest','attachment','price', 'readperm', 'highlight', 'views', 'replies' );
		$thread_json = array();
		foreach($threadids as $tableid => $tids) {
			foreach(C::t('forum_post')->fetch_all_by_tid($tableid, $tids, true, '', 0, 0, 1) as $post) {
				$_G['forum_threadlist'][$tableid]['message'] = messagecutstr($post['message'], 100); //需要用函数截取以下！不能显示太多内容
																								     //截取函数messagecutstr()
			}
			$thread_json = kouei_urlencode($_G['forum_threadlist'], $tableid, $encodearr, $thread_json);
		}
	}
}
...
if(defined('IN_MOBILE') && IN_MOBILE == 2){
	$threadlist_json = json_encode($thread_json);
	echo urldecode($threadlist_json);
}
