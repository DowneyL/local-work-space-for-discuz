...
var page = {
	converthtml : function() {
		...
		$('div.pg').removeClass('pg').addClass('page').html('<ul class="pager"><li class="previous"><a href="'+ prevpagehref +'">上一页</a></li>'+ selector +'<li class="next"><a href="'+ nextpagehref +'">下一页</a></li></ul>');
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				divobj = $(divobj);
				divobj.css({'position':'relative', 'margin-left':'-85px','margin-bottom':'10px'});
				$('.header').prepend(divobj);
