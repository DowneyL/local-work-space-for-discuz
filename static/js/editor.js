...
if(document.getElementById('message_talents')) {
var messageTalents = document.getElementById('message_talents').value;
}
	...
	if(wysiwyg) {
		...
		if(initialized && !(BROWSER.firefox && BROWSER.firefox >= '3' || BROWSER.opera)) {
			...
			if(messageTalents){
				...
				'<link rel="stylesheet" type="text/css" href="template/test/common/kouei_editor.css" />' +
				'</head><body>' + messageTalents + '</body></html>';
			}else{
				text = '<!DOCTYPE html PUBLIC "-/' + '/W3C/' + '/DTD XHTML 1.0 Transitional/' + '/EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' +
				'<html><head id="editorheader"><meta http-equiv="Content-Type" content="text/html; charset=' + charset + '" />' +
				(BROWSER.ie && BROWSER.ie > 7 ? '<meta http-equiv="X-UA-Compatible" content="IE=7" />' : '' ) +
